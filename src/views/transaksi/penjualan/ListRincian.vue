      ...
      <u-row gap="gap-0">
        <u-col align="items-end" gap="gap-1">
          <u-row gap="gap-2">
            ...
            <u-icon v-if="isHovered && !loadingHapusItem" name="delete" size="18"
              class="mb-0 text-danger cursor-pointer" @click.stop="handleDelete(item)" />
            <u-icon v-if="loadingHapusItem" name="loader"
              size="18"
              class="mb-0 text-primary animate-spin cursor-default"
              />
          </u-row>
          <u-row gap="gap-1">
            <u-text>Rp. </u-text>
