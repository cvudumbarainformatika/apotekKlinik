    ...
    <u-grid cols="5">
      ...
      <u-card class="col-span-2 h-full min-h-[100px] space-y-4">
        ...
        <!-- <u-grid cols="3" class="gap-2 mt-4" role="radiogroup"> -->
          <!-- <u-row class="">Diskon Keseluruhan: </u-row> -->
          <u-row class="">
            <u-input v-model="form.diskon_heder" label="Disc(%) Keseluruhan" :error="isError('diskon_heder')" type="number"
      ...
      <u-card class="col-span-3 h-full space-y-4">
        ...
        <u-grid v-if="form.hutang === 'HUTANG'" cols="3" class="gap-2 mt-4">
          <div>Jatuh Tempo :</div>
          <u-input-date class="col-span-2" type="date" v-model="form.tgl_jatuh_tempo" :error="errorMessage('tgl_jatuh_tempo')" />
        </u-grid>
        <!-- <u-grid cols="3" class="gap-2 mt-4" role="radiogroup">
          <u-row class="">Diskon Keseluruhan: </u-row>
          <u-row class="">
...
const TotalPenerimaan = computed(() => {
  if (props.store.mode === 'add') {
    ...
    // console.log('total xxx', props.store.form)
    const diskon_besar = Number(props.store.form?.diskon || 0)
    ...
    return items.reduce((a, b) => {
      ...
      const sub_subtotal = (jumlah_b * harga_b) - (jumlah_k * diskon_rp) + (jumlah_k * pajak_rp)
      const subtotal = sub_subtotal - ((sub_subtotal * diskon_besar) / 100)
      return a + subtotal
    }, 0)
  }
})
