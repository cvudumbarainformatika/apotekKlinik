...
<template>
  <u-page>
    ...
    <u-view class="w-full" padding="pt-1 px-2">
      <u-row flex1>
        <div class="font-bold text-lg text-primary">Laporan Fast Moving</div>
      </u-row>
    </u-view>
        ...
        <table class="w-full border-collapse">
          <thead class=" text-gray-700 text-sm uppercase">
            <tr>
              ...
              <th class="th text-right sticky-header">QTY JUAL</th>
              <th class="th text-right sticky-header">SUB HARGA JUAL</th>
              <th class="th text-right sticky-header">QTY RETUR</th>
              <th class="th text-right sticky-header">HARGA RETUR</th>
              <th class="th text-right sticky-header">TOTAL QTY</th>
              <th class="th text-right sticky-header">SUBTOTAL</th>
            </tr>
          </thead>
          <tbody>
            <template v-for="(item, i) in store.items" :key="i">
              ...
              <tr class="border-b last:border-0">
                <td class="td text-gray-600">
                   {{ item?.nama }}, ({{ item?.satuan_list }} )

                </td>
                <td class="text-right">{{ formatRupiah(item?.jumlah_penjualan) }}</td>
                <td class="text-right">{{ formatRupiah(item?.total_harga_jual) }}</td>
                <td class="text-right">{{ formatRupiah(item?.jumlah_retur) }}</td>
                <td class="text-right">{{ formatRupiah(item?.total_harga_retur) }}</td>
                <td class="text-right">{{ formatRupiah(item?.total_penjualan) }}</td>
                <td class="text-right">{{ formatRupiah(item?.total_harga_bersih) }}</td>
              </tr>
            </template>
          </tbody>
...
import { useLaporanFastmovingStore } from "@/stores/laporan/fastmoving"
...
const store = useLaporanFastmovingStore()
