<template>
    <u-modal persistent :title="`${title}`" @close="emit('close')" size="xl">
        <template #default>
            <div id="printData" class=" max-w-full bg-white text-black mx-auto  p-4 print-a4">
                <div class="flex items-start justify-between gap-6">
                    <div class="flex items-center gap-4">
                        <img src="/images/logo.png" alt="logo" class="w-14 h-14 object-contain" />
                        <div>
                            <div class="text-xl font-semibold tracking-wide">{{ company?.nama || 'Nama Apotik nya' }}
                            </div>
                        ...
                        <tbody>
                            <tr v-for="(item, index) in groupedItems" :key="index">
                                ...
                                <td class="td p-1 text-left align-top">
                                    ...
                                    <div
                                        v-if="item.jenis === 'SALDO AWAL' || item.jenis === 'PENERIMAAN' || item.jenis === 'RETUR PEMBELIAN'">
                    ...
                    <div class="rounded-xl p-4 bg-gray-50">
                        <div class="space-y-2 text-sm">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-semibold">{{ formatRupiah(totalPenerimaan) }} {{ store?.item?.satuan_k
                                    }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="font-semibold">Total Retur Penjualan</span>
                                <span class="font-semibold">{{ formatRupiah(totalReturPenjualan) }} {{
                                    store?.item?.satuan_k }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="font-semibold">Total Penyesuaian</span>
                                <span class="font-semibold">{{ formatRupiah(totalPenyesuaian) }} {{
                                    store?.item?.satuan_k }}</span>
                            </div>

                            <div class="flex items-center justify-between text-sm">
                                <span class="font-semibold">Total Penjualan</span>
                                <span class="font-semibold">{{ formatRupiah(totalPenjualan) }} {{ store?.item?.satuan_k
                                    }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="font-semibold">Total Retur Pembelian</span>
                                <span class="font-semibold">{{ formatRupiah(totalReturPembelian) }} {{
                                    store?.item?.satuan_k }}</span>
                            </div>
                            <div class="w-full border-t border-dotted border-black my-1"></div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="font-semibold">Total Stok</span>
                                <span class="font-semibold">{{ formatRupiah(totalStokAkhir) }} {{ store?.item?.satuan_k
                                    }}</span>
                            </div>
                        </div>
                    </div>
...
const periode = computed(() => {
    const bulanObj = bulans.value.find(b => b.value === props.store.params.bulan)
    const namaBulan = bulanObj ? bulanObj.label : ''
    const tahun = props.store.params.tahun || ''
...
const groupedItems = computed(() => {
    ...
    const saldoAwalRow = {
        jenis: 'SALDO AWAL',
        notrans: '-',
        tanggal: `${props.store.params.tahun}-${props.store.params.bulan}-01`,
...
const printObjx = {
    id: '#printData', // ref elemen yang mau diprint
    popTitle: 'Kartu Stok',
    preview: false,
    extraCss: '',
    extraHead: '',
