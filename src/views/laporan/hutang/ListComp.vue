    ...
    <div class="flex w-full items-start justify-between gap-6 mb-1">
      ...
      <div class="flex flex-col p-2">
        ...
        <div class="pt-2 uppercase text-sm font-bold text-right">
          TOTAL HUTANG : Rp. {{ formatRupiah(totalPembelian) }}
        </div>
      </div>
    </div>
    <table flex1 class="w-full text-sm border-separate [border-spacing:0]">
      <thead>
        <tr>
          ...
          <td class="td text-left p-1 font-bold">No Faktur</td>
          <td class="td text-right p-1 font-bold">Nilai</td>
        </tr>
      </thead>
      <tbody>
        <template v-for="(it, index) in store.items" :key="index">
          <tr>
            ...
            <td class="td p-1 align-top text-left">{{ it?.nofaktur || '-' }}
            </td>
            <td class="td p-1 align-top text-right">Rp. {{ formatRupiah(it?.total_rinci)|| '-' }}</td>
          </tr>
...
const totalPembelian = computed(() =>{
  return props.store.items.map((x) => parseInt(x.total_rinci)).reduce((a,b) => a+b, 0)
})
