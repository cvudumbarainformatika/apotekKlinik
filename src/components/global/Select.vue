...
<template>
  <div class="relative w-full autocomplete-wrapper" ref="wrapperRef">
    ...
    <Teleport to="body">
      <div v-show="open" class="absolute z-[9999] bg-secondary border rounded-lg shadow-lg max-h-60 overflow-auto"
        :style="dropdownStyle">
        <div v-for="(opt, idx) in props.options" :key="opt.value" @click="select(opt, idx)"
          class="px-4 py-2 text-xs cursor-pointer transition flex hover:bg-primary/10 hover:text-white" :class="{
    ...
    <!-- Error icon & message -->
    <div v-if="error" class="absolute inset-y-0 right-2 flex items-center gap-1 px-2 bg-transparent rounded-r-xl">
      <p class="text-xs text-danger whitespace-nowrap mr-2">
        {{ errorMessage }}
      </p>
      <div
        class="w-5 h-5 flex items-center justify-center text-danger border border-danger border-2 rounded-sm font-bold rotate-45">
